...
Typography is now powered by **CrimsonPro-Italic** (gratitude message — personal, editorial voice), **CrimsonPro-Regular** (org name, signer name, signer title), and **NothingYouCouldDo-Regular** (handwritten-feel signoff — "With gratitude," etc.) — bundled Google Fonts in `fonts/` rather than system Times New Roman.

Background is now **warm cream** (`#FCF9F3`) rather than pure white — physically closer to a letterpress card stock. A soft blurred ellipse in the accent color is composited in the upper-right corner to simulate a subtle watercolor brushstroke decoration (accent without clutter). The donor name salutation is set slightly oversize to make the recipient feel like the hero of the card. A thin decorative rule with a diamond center ornament separates the gratitude message from the signoff block. A gentle vignette (corner-darkening via composited `GaussianBlur` radial overlay) adds warmth and letterpress depth. Only a single restrained accent band at the top — no bottom band doubling it, which previously felt heavy.
