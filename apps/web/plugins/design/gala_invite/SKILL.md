...
Typography is now powered by **Italiana-Regular** (event name hero — tall, elegant display serif), **CrimsonPro-Regular** (tagline and venue text), and **WorkSans-Bold** (eyebrow "YOU ARE INVITED" label and date logistics in uppercase with letter-spacing) — bundled Google Fonts in `fonts/` rather than system Times/Helvetica.

Background is now a **subtle vertical gradient** (brand_color light-to-dark) rather than a flat solid, giving physical depth. A **corner-darkening vignette** is composited via `GaussianBlur` radial overlay — corners are notably darker than the center, creating the luminous focal-center feel of a stage spotlight. The double-border frame and corner diamond ornaments are preserved from the original but now rendered in Pillow with alpha compositing. The event name is rendered with a composited drop shadow (separate `GaussianBlur` layer) for dimension. Default accent color is now `#C9A961` (champagne gold) rather than `#D4AF37` — slightly warmer and more refined. The CTA is rendered as a stroke-outlined pill (no fill — ghosted) rather than plain text, which reads more elegantly against a dark background. Portrait format uses a 1.4× font scale factor for appropriate print sizing.
